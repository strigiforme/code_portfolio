...
module.exports = class Database {
  ...
  create_post(data) {
    ...
    return new Promise(  ( resolve, reject, postModel=this.postModel ) => {
      ...
      new_post.save( ( err, post ) => {
          if (err) { reject(err); }
          console.log("INFO: successfully uploaded post with id: '" + new_post._id + "'");
          resolve(new_post);
      });
    });
  }
    ...
    return new Promise(  ( resolve, reject, adminModel=this.adminModel ) => {
      ...
      admin_query.exec().then( admins => {
        if (admins.length > 1) {
          ...
          console.error("WARNING: More than one admin account is present. This currently should not be possible. "
  ...
  edit_post(id, new_post) {
    return new Promise(  ( resolve, reject, postModel=this.postModel ) => {
      ...
      edit_query.exec().then( post => {
        console.log("INFO: successfully edited post with id: '" + id + "' and update data: " + new_post);
        resolve(post);
      }).catch( err => {
        console.log("ERROR: unable to edit post with id: '" + id + "': " + err);
        reject(err);
      });
    });
  }
  ...
  post_fail(res, err) {
    console.log("ERROR: Unable to find the given post in mongodb.");
