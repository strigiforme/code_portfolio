...
module.exports = class Post {
  ...
  constructor(args) {
    ...
    if ( args.snippet == undefined || args.snippet == "undefined" ) {
      logger.log_debug("Setting snippet to be undefined in post constructor");
      this.snippet = undefined;
    } else {
      logger.log_debug(`Setting snippet to be '${args.snippet}' post in constructor`);
  ...
  set_type(new_type) {
    ...
    for (var type in post_types) {
      if ( new_type == type ) {
        logger.log_debug(`Provided post type exists, setting post type to be value '${new_type}'`)
        this.type = new_type;
        exists = true;
      }
    }
    if (!exists) {
      logger.log_debug(`Setting post type to be unknown, type '${new_type}' was not recognized.`);
  ...
  clean() {
    ...
    logger.log_debug("Cleaning type '" + this.type + "' -> '" + Sanitizer.clean(this.type) + "'");
    this.type    = Sanitizer.clean(this.type);
    logger.log_debug("Cleaning content '" + this.content + "' -> '" + Sanitizer.clean(this.content) + "'");
    this.content = Sanitizer.clean(this.content);
    logger.log_debug("Cleaning title '" + this.title + "' -> '" + Sanitizer.clean(this.title) + "'");
    this.title   = Sanitizer.clean(this.title);
  }

  // make content presentable for user
  unescape() {
    logger.log_debug("Unescaping post data:");
    logger.log_debug("Unescaping type '" + this.type + "' -> '" + unescape(this.type) + "'");
    this.type    = Sanitizer.prepare(this.type);
    logger.log_debug("Unescaping content '" + this.content + "' -> '" + unescape(this.content) + "'");
    this.content = Sanitizer.prepare(this.content);
    logger.log_debug("Splitting content '" + this.content + "' -> '" + this.content.split("\n") + "'");
    this.content = this.content.split("\n");
    logger.log_debug("Unescaping title '" + this.title + "' -> '" + unescape(this.title) + "'");
