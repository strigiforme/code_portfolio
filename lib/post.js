...
module.exports = class Post {
  ...
  set_type(new_type) {
    ...
    for (var type in post_types) {
      if ( new_type == type ) {
        console.log("DEBUG: New post type exists, setting post type to be value '" + new_type + "'")
        this.type = new_type;
        exists = true;
      }
    }
    if (!exists) {
      console.log("DEBUG: Setting post type to be unknown, type '" + new_type + "' was not recognized.");
